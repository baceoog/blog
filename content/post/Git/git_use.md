...
#重命名当前分支为main
git branch -m main
#强制推送当前分支到远程仓库, 覆盖原仓库的所有东西且**该操作不可逆**
...
前面的一串数字是什么意思
答：
前面的一串数字是哈希值，它是一个由 40 个十六进制字符组成的字符串，用来唯一标识每一次提交。
Git 通过计算提交的内容、作者、日期等信息的 SHA-1 散列值来生成这个哈希值
```
...
[2] git pull and git pull --rebase  
